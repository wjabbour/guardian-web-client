...
class Dynamo {
  ...
  async setPaid(paypal_order_id: string, txId: string) {
    const command = new QueryCommand({
      ...
      KeyConditionExpression: "paypal_order_id = :paypal_order_id",
      TableName: "orders",
      IndexName: "paypal-order-id-index",
    });
  ...
  async deleteOrder(created_at: string, email: string) {
    const deleteOrder = new DeleteCommand({
      Key: {
        created_at,
        email,
      },
    ...
    await this.documentClient.send(deleteOrder);
    ...
    await this.documentClient.send(deleteArchivedOrder);
